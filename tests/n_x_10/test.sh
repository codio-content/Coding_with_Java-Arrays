#!/bin/bash

N=3
M=3
INPUT0=7
INPUT1=2
INPUT2=5
INPUT3=1

echo "<h3>Program Output</h3>"

cd /home/codio/workspace/challenges/n_x_10

javac Challenge.java
if [ $? -ne 0 ]; then exit 1; fi

# 1
OUTPUT1=($(java Challenge $N $M $INPUT0 $INPUT1 $INPUT2 $INPUT3))
if [ $? -ne 0 ]; then exit 1; fi

echo "Input: N = " $N ", M = " $M  ", ints = " $INPUT0 $INPUT1 $INPUT2 $INPUT3 

echo "Output: "
for i in ${OUTPUT1[@]}; do
  echo $i
done

if [ ${#OUTPUT1[@]} != 4 ]; then
  echo "<br/><hr/><h3>Challenge Feedback</h3>"
  echo "Your code is not outputing the expected number of values"
  exit 1
fi

COUNT=$N-1
ARR[0]=$INPUT0
ARR[1]=$INPUT1
ARR[2]=$INPUT2
ARR[3]=$INPUT3

while (( COUNT < ${#ARR[@]} ))
do
  ARR[$COUNT]=$((ARR[COUNT]*M))
  (( COUNT=$COUNT+$N ))
done

COUNT2=0
for i in ${ARR[@]}; 
do
  arg=$( sed -e "s@\[@@g" <<< "${OUTPUT1[COUNT2]}" )
  arg=$( sed -e "s@\]@@g" <<< "$arg" )
  arg=$( sed -e "s@,@@g" <<< "$arg" )
  if [ "$arg" != "$i" ]; then
    echo "<br/><hr/><h3>Challenge Feedback</h3>"
    echo "Your code is not outputing the correct value"
    exit 1
  fi
  (( COUNT2++ ))
done

echo "<br/><hr/><h3>Challenge Feedback</h3>"
echo "Well done you passed the challenge!"
exit 0

